...
function getClientInfo(request) {
  ...
  // Security checks
  const isSecureConnection = hdrs.get('x-forwarded-proto') === 'https' || 
                            hdrs.get('x-forwarded-ssl') === 'on' ||
                            hdrs.get('cf-visitor')?.includes('https');
  
  // Basic risk assessment
  let riskScore = 0;
  if (!isSecureConnection) riskScore += 20;
  if (ip.startsWith('127.') || ip === '::1') riskScore += 5; // Local IP
  if (/bot|crawler|spider|crawling/i.test(userAgent)) riskScore += 30;
...
export async function createSession(userId, request, loginSource = 'direct') {
  try {
    ...
    const [existing] = await user_db.promise().query(
    ...
    if (existing.length > 0) {
      ...
      await user_db.promise().query(
        `UPDATE user_sessions 
         SET session_token = ?, refresh_token = ?, browser_name = ?, browser_version = ?,
             os_name = ?, os_version = ?, device_type = ?, is_secure_connection = ?,
             risk_score = ?, login_source = ?, referrer_url = ?, landing_page = ?,
             last_activity = NOW(), expires_at = ?
         WHERE id = ?`,
        [
          accessToken, refreshToken, clientInfo.browserName, clientInfo.browserVersion,
          clientInfo.osName, clientInfo.osVersion, clientInfo.deviceType, clientInfo.isSecureConnection,
          clientInfo.riskScore, loginSource, request.headers.get('referer') || '', request.url || '',
      ...
      await user_db.promise().query(
        `UPDATE user_sessions SET is_active = 0, logout_time = NOW(), logout_reason = 'replaced'
    ...
    const [result] = await user_db.promise().query(`
      INSERT INTO user_sessions (
        ...
        is_secure_connection, risk_score, expires_at, login_source,
    ...
    `, [
      ...
      clientInfo.ipAddress, clientInfo.isSecureConnection, clientInfo.riskScore, expiresAt,
      loginSource, request.headers.get('referer') || '', request.url || ''
    ]);
  ...
  try {
    ...
    await user_db.promise().query(`
      INSERT INTO login_analytics (
        user_id, session_id, login_date, login_hour, login_day_of_week,
        device_type, browser_name, os_name, ip_address, risk_level
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      userId, sessionId, now.toISOString().split('T')[0], now.getHours(), now.getDay(),
      clientInfo.deviceType, clientInfo.browserName, clientInfo.osName, clientInfo.ipAddress,
      clientInfo.riskScore > 50 ? 'high' : clientInfo.riskScore > 20 ? 'medium' : 'low'
    ]);
...
export async function validateSession(sessionToken) {
  try {
    ...
    const [sessions] = await user_db.promise().query(`
      SELECT s.*, u.username, u.first_name, u.last_name, u.email, u.active, u.deleted
      FROM user_sessions s
      JOIN user_db u ON s.user_id = u.id
  ...
  try {
    const [sessions] = await user_db.promise().query(`
      SELECT id, device_type, browser_name, os_name, ip_address, 
             login_time, last_activity, is_active, risk_score
      FROM user_sessions 
      WHERE user_id = ? 
  ...
  try {
    ...
    await user_db.promise().query(`
      INSERT INTO security_events (
        user_id, session_id, event_type, event_severity, event_description,
        ip_address, user_agent, risk_score
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      userId, sessionId, eventType, eventSeverity, description,
      clientInfo.ipAddress || null, clientInfo.userAgent || null, clientInfo.riskScore || 0
    ]);
